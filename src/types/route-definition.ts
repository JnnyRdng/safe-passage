...
type OptionalParams<S extends Params> = {
  [K in keyof S as S[K]["optional"] extends true ? K : never]?: ParamToTS<S[K]>;
};

type Exact<T, U extends keyof any = keyof T> = T & {
  [K in Exclude<string, U>]?: never;
};

export type InferSearchParams<S extends Params | undefined> =
  S extends Params ? RequiredParams<S> & OptionalParams<S> : {};
